...
class DictField[K:Hashable, V](ContainerField[Mapping[K, V]]):
    ...
    def __eq__(self, value: object) -> bool:
        return isinstance(value, type(self)) and (len(self.field_slots) == 0 and len(value.field_slots) == 0 and len(self.variable_slots) == 0 and len(value.variable_slots) == 0 or self.optional_scope == value.optional_scope) \
            and self.field_slots == value.field_slots

    def __hash__(self) -> int:
        return hash((type(self), (self.optional_scope if len(self.field_slots) > 0 or len(self.variable_slots) > 0 else None), tuple(self.field_slots)))
