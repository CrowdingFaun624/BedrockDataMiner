...
from _domains.minecraft_common.scripts.Nbt.NbtExceptions import SnbtParseError
...
def parse_compound(reader:Reader) -> "NbtTypes.TAG_Compound":
    ...
    if reader.read() != "{":
        raise SnbtParseError("Expected \"{\"", reader)
    if reader.read() == "}":
    ...
    while True:
        ...
        match reader.read():
            ...
            case "}":
                break
            case "":
                raise SnbtParseError("Compound did not end before end", reader)
            case _:
                raise SnbtParseError("Expected \",\" or \"}\"", reader)
    ...
    while True:
        ...
        if first_type is None:
            ...
            if not isinstance(item, first_type):
                raise SnbtParseError(f"List type {index} type \"{item.__class__.__name__}\" does not match first item type \"{first_type.__name__}\"", reader)
        parse_whitespace(reader)
        match reader.read():
            case ",":
                continue # next item
            case "]":
                break
            case "":
                raise SnbtParseError("List did not end before end", reader)
            case _:
                raise SnbtParseError("Expected \",\" or \"]\"", reader)
...
def parse_byte_array(reader:Reader) -> "NbtTypes.TAG_Byte_Array":
    ...
    parse_whitespace(reader)
    if reader.read(2) != "B;":
        raise SnbtParseError("Expected \"B;\"", reader)
    if reader.read() == "]":
    ...
    while True:
        ...
        match reader.read():
            ...
            case "]":
                break
            case "":
                raise SnbtParseError("Byte array did not end before end", reader)
            case _:
                raise SnbtParseError("Expected \",\" or \"]\"", reader)
...
def parse_int_array(reader:Reader) -> "NbtTypes.TAG_Int_Array":
    ...
    parse_whitespace(reader)
    if reader.read(2) != "I;":
        raise SnbtParseError("Expected \"I;\"", reader)
    if reader.read() == "]":
    ...
    while True:
        ...
        match reader.read():
            ...
            case "]":
                break
            case "":
                raise SnbtParseError("Int array did not end before end", reader)
            case _:
                raise SnbtParseError("Expected \",\" or \"]\"", reader)
...
def parse_long_array(reader:Reader) -> "NbtTypes.TAG_Long_Array":
    ...
    parse_whitespace(reader)
    if reader.read(2) != "L;":
        raise SnbtParseError("Expected \"L;\"", reader)
    if reader.read() == "]":
    ...
    while True:
        ...
        match reader.read():
            ...
            case "]":
                break
            case "":
                raise SnbtParseError("Long array did not end before end", reader)
            case _:
                raise SnbtParseError("Expected \",\" or \"]\"", reader)
...
def parse_byte(reader:Reader) -> "NbtTypes.TAG_Byte":
    ...
    while True:
        ...
        if character.isalpha():
            if len(characters) == 0:
                raise SnbtParseError("Expected digit", reader)
            if character.lower() == "b":
                break
            else:
                raise SnbtParseError("Expected \"b\" or \"B\"", reader)
        elif character.isdigit():
            characters.append(character)
        elif character == ".":
            raise SnbtParseError("Decimal points are invalid in bytes", reader)
        else:
            raise SnbtParseError("Expected digit, \"b\", or \"B\"", reader)
    output = int("".join(characters))
    if is_negative: output = -output
    if output > 127:
        raise SnbtParseError("Byte value is greater than 127", reader)
    elif output < -128:
        raise SnbtParseError("Byte value is less than -128", reader)
    ...
    while True:
        ...
        if character.isalpha():
            ...
            if character.lower() == "s":
                ...
                raise SnbtParseError("Expected \"s\" or \"S\"", reader)
        elif character.isdigit():
            characters.append(character)
        elif character == ".":
            raise SnbtParseError("Decimal points are invalid in shorts", reader)
        else:
            raise SnbtParseError("Expected digit, \"s\", or \"S\"", reader)
    output = int("".join(characters))
    if is_negative: output = -output
    if output > 32767:
        raise SnbtParseError("Short value is greater than 32767", reader)
    elif output < -32768:
        raise SnbtParseError("Short value is less than -32768", reader)
...
def parse_int(reader:Reader) -> "NbtTypes.TAG_Int":
    ...
    while True:
        ...
        if character.isdecimal():
            ...
            raise SnbtParseError("Decimal points are invalid in ints", reader)
        else:
            reader.back()
            if len(characters) == 0:
                raise SnbtParseError("Expected digit", reader)
    ...
    if output > 2147483647:
        raise SnbtParseError("Int value is greater than 2147483647", reader)
    elif output < -2147483648:
        raise SnbtParseError("Int value is less than -2147483648", reader)
...
def parse_long(reader:Reader) -> "NbtTypes.TAG_Long":
    ...
    while True:
        ...
        if character.isdecimal():
            ...
            raise SnbtParseError("Decimal points are invalid in longs", reader)
        else:
            if character.lower() == "l":
                ...
                raise SnbtParseError("Expected \"l\" or \"L\"", reader)
    output = int("".join(characters))
    if is_negative: output = -output
    if output > 9223372036854775807:
        raise SnbtParseError("Long value is greater than 9223372036854775807", reader)
    elif output < -9223372036854775808:
        raise SnbtParseError("Long value is less than -9223372036854775808", reader)
    ...
    while True:
        ...
        if character.isdecimal():
            ...
            if len(characters_pre_decimal) == 0:
                raise SnbtParseError("Expected digits before decimal", reader)
            break
        else:
            raise SnbtParseError("Expected digit", reader)
    while True:
        ...
        if character.isdecimal():
            ...
            raise SnbtParseError("Expected digit, \"f\", or \"F\", not additional decimal point", reader)
        elif character.isalpha():
            if character.lower() == "f":
                if len(characters_post_decimal) == 0:
                    raise SnbtParseError("Expected digits after decimal", reader)
                break
            else:
                raise SnbtParseError("Expected digit", reader)
        else:
...
def parse_double(reader:Reader) -> "NbtTypes.TAG_Double":
    ...
    while True:
        ...
        if character.isdecimal():
            ...
            raise SnbtParseError("Expected digit, \"d\", or \"D\", not additional decimal point", reader)
        else:
            if len(characters_post_decimal) == 0:
                raise SnbtParseError("Expected digits after decimal", reader)
...
def parse_string(reader:Reader, allow_unquoted_strings:bool=False) -> "NbtTypes.TAG_String":
    ...
    if character in "\"'":
        ...
        if allow_unquoted_strings:
            ...
            raise SnbtParseError("Unquoted strings are not allowed", reader)
    is_escaped = False
    output = ""
    while True:
        character = reader.read()
        if len(character) == 0:
            raise SnbtParseError("String did not end before end", reader)
        if is_quoted:
            quote_type = cast(str, quote_type)
            if is_escaped:
                if character == "\\": output += "\\"
                elif character == quote_type: output += quote_type
                else: raise SnbtParseError(f"Expected escaped character to be \"\\\" or \"{quote_type}\"", reader)
                is_escaped = False
            else:
            ...
            if character.isalnum() or character in "_-.+":
                ...
                if len(output) == 0:
                    raise SnbtParseError("Expected character", reader)
                reader.back()
                break
...
def parse_thing(reader:Reader) -> "NbtTypes.TAG":
    ...
    exceptions:list[SnbtParseError] = []
    for function in functions:
        ...
        raise SnbtParseError("Unknown data", reader, exceptions)

def parse(data:str) -> "NbtTypes.TAG":
    reader = Reader(data)
    output = parse_thing(reader)
    if not reader.is_at_end():
        raise SnbtParseError("Trailing data", reader)
    return output
