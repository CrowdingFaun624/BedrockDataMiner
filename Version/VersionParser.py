...
def verify_ordering(versions:dict[str,Version.Version], version_tags:VersionTags.VersionTags) -> None:
    '''Raises an error if the ordering of the versions is incorrect in any way.'''
    ...
    for version in versions.values():
        if (order_tag_count := sum(order_tag in version.tags for order_tag in ORDERING_TAGS)) != 1:
            raise Exceptions.VersionOrderingTagsError(version, order_tag_count, version.tags)
    ...
    if len(set(top_level_childrens)) != len(top_level_childrens):
        ...
        for version in top_level_childrens:
            if version in already_seen_versions: raise Exceptions.VersionChildOfMultipleTopLevelVersionsError(version)
    ...
    for version in top_level_versions:
        if TOP_LEVEL_TAG not in version.tags:
            raise Exceptions.VersionTopLevelError(version, TOP_LEVEL_TAG)
    for version in versions.values():
        ...
        for child in version.children:
            ...
            if child_order_tag not in ALLOWED_CHILDREN[version_order_tag]:
                raise Exceptions.VersionChildError(version, version_order_tag, child, child_order_tag)
        ...
        for child in version.children:
            while not order_contains_at_index(child.get_order_tag()):
                ...
                if order_index >= len(ORDER):
                    raise Exceptions.VersionChildOrderError(version, [child.get_order_tag() for child in version.children], child)
        ...
        for child in version.children:
            if child.get_order_tag() in BEFORE_TAGS:
                if version.time is not None and child.time is not None and child.time > version.time:
                    raise Exceptions.VersionOrderSequenceError(version, version.get_order_tag(), child, child.get_order_tag(), "after")
            elif child.get_order_tag() in AFTER_TAGS:
                if version.time is not None and child.time is not None and child.time < version.time:
                    raise Exceptions.VersionOrderSequenceError(version, version.get_order_tag(), child, child.get_order_tag(), "before")
            if previous_time is not None:
                if previous_child is None:
                    raise Exceptions.InvalidStateError("previous_child is None but previous_time is not None!")
                if child.time is not None and child.time < previous_time:
                    raise Exceptions.VersionTimeTravelError(previous_child, previous_time, child, child.time, version)
...
def assign_latest(versions:dict[str,Version.Version], version_tags:VersionTags.VersionTags) -> None:
    for latest_slot, latest_slot_tags in version_tags.latest.latest_tags.items():
        for version in reversed(versions.values()):
            if version.get_order_tag() in latest_slot_tags and len(version.version_files) > 0:
                version.latest = True
