...
def collapse_resource_packs(properties:dict[str,Any], resource_packs:list["DataMinerTyping.ResourcePackTypedDict"], version_name:str, add_defined_in:bool=True) -> dict[str,Any]:
    ...
    for properties_resource_pack in properties:
        if properties_resource_pack not in resource_pack_names:
            raise KeyError("Unknown resource pack \"%s\" on Blocks normalizer in \"%s\"!" % (properties_resource_pack, version_name))
