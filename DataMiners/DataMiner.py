...
class DataMinerCollection():
    ...
    def compare(self, version1:Version.Version|None, version2:Version.Version, versions_between:list[Version.Version], all_dataminers:list["DataMinerCollection"], store:bool=True, data_cache:dict[Version.Version,Any]|None=None) -> str:
        ...
        if version1 is None:
            version2_data = data_cache[version2] = data_cache[version2] if data_cache is not None and version2 in data_cache else self.get_data_file(version2)
            version1_data = type(version2_data)() # create new empty object.
        else:
            version1_data = data_cache[version1] = data_cache[version1] if data_cache is not None and version1 in data_cache else self.get_data_file(version1)
            version2_data = data_cache[version2] = data_cache[version2] if data_cache is not None and version2 in data_cache else self.get_data_file(version2)
