...
class Structure(Generic[a]):
    ...
    def print_single(self, key_str:str|int|None, data:a, message:str, output:list[SU.Line], printer:Union["Structure[a]",None], trace:Trace.Trace) -> None:
        if printer is None:
            ...
            if key_str is None:
                output.append(SU.Line("%s %s %s.") % (message, self.name, stringified_data))
            else:
                output.append(SU.Line("%s %s %s of %s.") % (message, self.name, SU.stringify(key_str), stringified_data))
        else:
            substructure_output = printer.print_text(data, trace.copy(self.name, key_str))
            match len(substructure_output), key_str is None:
                case 0, True:
                    output.append(SU.Line("%s empty %s.") % (message, self.name))
                case 0, False:
                    output.append(SU.Line("%s empty %s %s.") % (message, self.name, SU.stringify(key_str)))
                case 1, True:
                    output.append(SU.Line("%s %s %s.") % (message, self.name, substructure_output[0]))
                case 1, False:
                    output.append(SU.Line("%s %s %s of %s.") % (message, self.name, SU.stringify(key_str), substructure_output[0]))
                case _, True:
                    output.append(SU.Line("%s %s:") % (message, self.name))
                    output.extend(line.indent() for line in substructure_output)
                case _, False:
                    output.append(SU.Line("%s %s %s:") % (message, self.name, SU.stringify(key_str)))
                    output.extend(line.indent() for line in substructure_output)
    ...
    def print_double(self, key_str:str|int|None, data1:a, data2:a, message:str, output:list[SU.Line], printers:"StructureSet.StructureSet", trace:Trace.Trace) -> None:
        ...
        match len(substructure_output1), len(substructure_output2), key_str is None:
            case 1, 1, True:
                output.append(SU.Line("%s %s from %s to %s.") % (message, self.name, substructure_output1[0], substructure_output2[0]))
            case 1, 1, False:
                output.append(SU.Line("%s %s %s from %s to %s.") % (message, self.name, SU.stringify(key_str), substructure_output1[0], substructure_output2[0]))
            case 1, _, True:
                output.append(SU.Line("%s %s from %s to:") % (message, self.name, substructure_output1[0]))
                output.extend(line.indent() for line in substructure_output2)
            case 1, _, False:
                output.append(SU.Line("%s %s %s from %s to:") % (message, self.name, SU.stringify(key_str), substructure_output1[0]))
                output.extend(line.indent() for line in substructure_output2)
            case _, 1, True:
                output.append(SU.Line("%s %s to %s from:") % (message, self.name, substructure_output2[0]))
                output.extend(line.indent() for line in substructure_output1)
            case _, 1, False:
                output.append(SU.Line("%s %s %s to %s from:") % (message, self.name, SU.stringify(key_str), substructure_output2[0]))
                output.extend(line.indent() for line in substructure_output1)
            case _, _, True:
                output.append(SU.Line("%s %s from:") % (message, self.name))
                output.extend(line.indent() for line in substructure_output1)
                output.append(SU.Line("to:"))
                output.extend(line.indent() for line in substructure_output2)
            case _, _, False:
                output.append(SU.Line("%s %s %s from:") % (message, self.name, SU.stringify(key_str)))
                output.extend(line.indent() for line in substructure_output1)
                output.append(SU.Line("to:"))
                output.extend(line.indent() for line in substructure_output2)
