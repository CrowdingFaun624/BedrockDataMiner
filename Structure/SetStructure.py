...
class SetStructure(AbstractIterableStructure.AbstractIterableStructure[d]):
    ...
    def get_similarity(self, data1:Sequence[d], data2:Sequence[d]) -> float:
        data1_hashes:dict[int,d] = {Hashing.hash_data(item): item for item in data1}
        data2_hashes:dict[int,d] = {Hashing.hash_data(item): item for item in data2}
        ...
        if self.structure is not None and len(data1_exclusive_items) > 0 and len(data2_exclusive_items) > 0:
            ...
            for hash1, hash2, item_similarity in self.get_similarities_list(data1_exclusive_items, data2_exclusive_items, self.structure):
    ...
    def get_similarities_list(self, data1_exclusive_items:dict[int,d], data2_exclusive_items:dict[int,d], structure:Structure.Structure[d]) -> list[tuple[int,int,float]]:
        similarities_list = [ # maps similarity of older items to newer items
            (hash1, hash2, similarity)
            for hash1, item1 in data1_exclusive_items.items()
    ...
    def compare(
        ...
        for same_hash in same_hashes:
            output.append(data1_hashes[same_hash])
        ...
        if self.structure is not None and len(data1_exclusive_items) > 0 and len(data2_exclusive_items) > 0:
            for hash1, hash2, similarity in self.get_similarities_list(data1_exclusive_items, data2_exclusive_items, self.structure):
                if hash1 in already_data1_hashes or hash2 in already_data2_hashes:
                    continue # if either side is already involved in a change, it's unneeded.
                already_data1_hashes.add(hash1)
                already_data2_hashes.add(hash2)
                compare_output, _, new_exceptions = self.structure.compare(data1_exclusive_items[hash1], data2_exclusive_items[hash2], environment)
        ...
        output.extend(
            D.Diff(old=item)
            for exclusive_hash, item in data1_exclusive_items.items()
            if exclusive_hash not in already_data1_hashes
        )
        output.extend(
            D.Diff(new=item)
            for exclusive_hash, item in data2_exclusive_items.items()
